...
async def resolve_cn_tags(
    ...
    logger.debug("[resolve_cn_tags] NER: %s", ner_result)
    ...
    # 构建中文 → 英文 tag 的映射（只包含成功匹配的）
    entity_mapping = {
        tag.original_name: tag.en_tag
        for tag in retrieval.resolved
        if not tag.fallback_nl
    }

    logger.debug("[resolve_cn_tags] confirmed=%s nltags=%s mapping=%s", confirmed, nltags, entity_mapping)
    if nltags:
        logger.info(
            "[resolve_cn_tags] 原文 %r → confirmed=%s, nltags=%s",
            user_text[:50], confirmed, nltags,
        )
    return confirmed, nltags, negative, entity_mapping
